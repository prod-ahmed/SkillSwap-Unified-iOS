//
//  WeeklyObjectiveWidgetExtension.swift
//  WeeklyObjectiveWidgetExtension
//
//  Created by Ahmed BT on 08/12/2025.
//
...
struct WeeklyObjectiveProvider: TimelineProvider {
    typealias Entry = WeeklyObjectiveEntry
    ...
    func getSnapshot(in context: Context, completion: @escaping (WeeklyObjectiveEntry) -> Void) {
        Task {
            let objective = await fetchCurrentObjective()
            let entry = WeeklyObjectiveEntry(date: Date(), objective: objective)
            completion(entry)
        }
    }
    
    func getTimeline(in context: Context, completion: @escaping (Timeline<WeeklyObjectiveEntry>) -> Void) {
        Task {
            let objective = await fetchCurrentObjective()
            let entry = WeeklyObjectiveEntry(date: Date(), objective: objective)
    ...
    private func fetchCurrentObjective() async -> WidgetObjective? {
        ...
        let baseURL = sharedDefaults?.string(forKey: "baseURL") ?? "http://127.0.0.1:3000"
...
// MARK: - Widget View
struct WeeklyObjectiveWidgetExtensionEntryView: View {
...
struct WeeklyObjectiveWidgetExtension: Widget {
    let kind: String = "WeeklyObjectiveWidgetExtension"
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(
            ...
            WeeklyObjectiveWidgetExtensionEntryView(entry: entry)
